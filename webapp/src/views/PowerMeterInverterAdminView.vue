<template>
    <BasePage :title="$t('powermeterinverteradmin.PowerMeterSettings')" :isLoading="dataLoading">
        ...
        <form @submit="savePowerMeterConfig">
            <CardElement :text="$t('powermeteradmin.PowerMeterConfiguration')" textVariant="text-bg-primary">
                <InputElement
                    ...
                    v-model="PowerMeterInverterConfigList.enabled"
                    type="checkbox"
                    wide
                />

                <InputElement
                    v-show="PowerMeterInverterConfigList.enabled"
                    :label="$t('powermeteradmin.VerboseLogging')"
                    v-model="PowerMeterInverterConfigList.verbose_logging"
                    type="checkbox"
                    wide
                />

                <div class="row mb-3" v-show="PowerMeterInverterConfigList.enabled">
                    <label for="inputPowerMeterSource" class="col-sm-4 col-form-label">{{
                        $t('powermeteradmin.PowerMeterSource')
                    }}</label>
                    <div class="col-sm-8">
                        <select id="inputPowerMeterSource" class="form-select" v-model="PowerMeterInverterConfigList.source">
            ...
            <div v-if="PowerMeterInverterConfigList.enabled">
                <div v-if="PowerMeterInverterConfigList.source === 0 || PowerMeterInverterConfigList.source === 3">
                ...
                <div v-if="PowerMeterInverterConfigList.source === 0">
                    <CardElement
                        v-for="(mqtt, index) in PowerMeterInverterConfigList.mqtt.values"
                        v-bind:key="index"
                ...
                <CardElement
                    v-if="PowerMeterInverterConfigList.source === 1 || PowerMeterInverterConfigList.source === 2"
                ...
                >
                    <InputElement
                        ...
                        v-model="PowerMeterInverterConfigList.serial_sdm.polling_interval"
                        type="number"
                        min="1"
                        max="15"
                    ...
                    <InputElement
                        ...
                        v-model="PowerMeterInverterConfigList.serial_sdm.address"
                        type="number"
                        wide
                    />
                </CardElement>

                <div v-if="PowerMeterInverterConfigList.source === 3">
                    <div class="alert alert-secondary mt-5" role="alert">
                    ...
                    <CardElement :text="$t('powermeteradmin.HTTP')" textVariant="text-bg-primary" add-space>
                        <InputElement
                            ...
                            v-model="PowerMeterInverterConfigList.http_json.individual_requests"
                            type="checkbox"
                            wide
                        />
                        ...
                        <InputElement
                            :label="$t('powermeteradmin.pollingInterval')"
                            v-model="PowerMeterInverterConfigList.http_json.polling_interval"
                            type="number"
                            min="1"
                            max="15"
                            :postfix="$t('powermeteradmin.seconds')"
                    ...
                    <CardElement
                        v-for="(httpJson, index) in PowerMeterInverterConfigList.http_json.values"
                        :key="index"
                    ...
                    >
                        ...
                        <div v-if="httpJson.enabled || index == 0">
                            <HttpRequestSettings
                                ...
                                v-if="index == 0 || PowerMeterInverterConfigList.http_json.individual_requests"
                            />
                ...
                <div v-if="PowerMeterInverterConfigList.source === 6">
                    <CardElement :text="$t('powermeteradmin.HTTP_SML')" textVariant="text-bg-primary" add-space>
                        <InputElement
                            :label="$t('powermeteradmin.pollingInterval')"
                            v-model="PowerMeterInverterConfigList.http_sml.polling_interval"
                            type="number"
                            min="1"
                            max="15"
                            :postfix="$t('powermeteradmin.seconds')"
                        ...
                        <HttpRequestSettings v-model="PowerMeterInverterConfigList.http_sml.http_request" />
                    </CardElement>
...
export default defineComponent({
    ...
    data() {
        return {
            dataLoading: true,
            PowerMeterInverterConfigList: {} as PowerMeterConfig,
    ...
    methods: {
        getPowerMeterConfig() {
            ...
            fetch('/api/powermeterinverter/config', { headers: authHeader() })
                .then((response) => handleResponse(response, this.$emitter, this.$router))
                .then((data) => {
                    this.PowerMeterInverterConfigList = data;
        ...
        savePowerMeterConfig(e: Event) {
            ...
            formData.append('data', JSON.stringify(this.PowerMeterInverterConfigList));

            fetch('/api/powermeterinverter/config', {
                method: 'POST',
                headers: authHeader(),
                body: formData,
            })
        ...
        testHttpJsonRequest() {
            ...
            fetch('/api/powermeterinverter/testhttpjsonrequest', {
                method: 'POST',
                headers: authHeader(),
                body: formData,
            })
        ...
        testHttpSmlRequest() {
            ...
            fetch('/api/powermeterinverter/testhttpsmlrequest', {
                method: 'POST',
                headers: authHeader(),
                body: formData,
            })
